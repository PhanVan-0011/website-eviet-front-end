...
const SliderUpdate = () => {
    ...
    useEffect(() => {
        ...
        const fetchSlider = async () => {
            try {
                ...
                setValue('display_order', data.display_order || 1);
                setDisplayOrder(data.display_order || 1);
                setValue('is_active', data.is_active === true ? "1" : "0");
                
                // Xử lý ảnh từ image_urls array
                if (data.image_urls && data.image_urls.length > 0) {
                    const featuredImg = data.image_urls.find(img => img.is_featured === 1);
                    const firstImg = featuredImg || data.image_urls[0];
                    const imageUrl = firstImg.thumb_url || firstImg.main_url;
                ...
                setLinkType(type);
                setValue('link_type', type);
                setLinkId(data.linkable_id ? String(data.linkable_id) : '');
                setValue('link_id', data.linkable_id ? String(data.linkable_id) : '');
                 dispatch(actions.controlLoading(false));
            } catch (error) {
                 dispatch(actions.controlLoading(false));
                toast.error("Không lấy được dữ liệu slider", toastErrorConfig);
            }
        };
        fetchSlider();
    }, [params.id, setValue]);
    ...
    const onChangeImage = (e) => {
        ...
        clearErrors('imageFile'); // Clear error khi chọn ảnh mới
        e.target.value = "";
    };
    ...
    const handleSubmitForm = async (data) => {
        // Kiểm tra nếu không có ảnh nào (cả mới và cũ)
        if (!imageFile && !oldImage) {
            setError('imageFile', { type: 'manual', message: 'Ảnh slider là bắt buộc' });
            return;
        }
        ...
        try {
            ...
            if (e.response && e.response.data && e.response.data.message) {
                ...
                toast.error("Lỗi khi cập nhật slider", toastErrorConfig);
            }
        } finally {
        ...
        <div id="layoutSidenav_content">
            <main>
                <div className="container-fluid px-4">
                    <h1 className="mt-4">Cập nhật slider</h1>
                    <ol className="breadcrumb mb-4 p-2">
                        <li className="breadcrumb-item"><Link to="/">Trang chủ</Link></li>
                        <li className="breadcrumb-item active">Cập nhật slider</li>
                    </ol>
                                        ...
                                        <div className="mb-3">
                                            <div className="d-flex flex-column align-items-center">
                                                ...
                                                <label htmlFor="inputImage" className="form-label btn btn-secondary mb-0 mt-2">
                                                    <i className="fas fa-upload"></i> {imagePreview || oldImage ? 'Thay đổi ảnh' : 'Thêm ảnh slider'}
                                                </label>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                ...
                <Modal.Body>
                    <p>Bạn chắc chắn muốn xóa slider này?</p>
                </Modal.Body>
                <Modal.Footer>
